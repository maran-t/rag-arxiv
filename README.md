...
QDRANT_COLLECTION=arxiv_vector_store03
```

### 3. Dataset
This repo have a file:
```
arxiv_data.csv
```
...
What it does:
- Loads CSV with `CSVLoader`.
- Cleans + reformats entries:
  ```
  Title: <paper_title>
...
- Saves vectors to Qdrant collection For now, creates a vector for each row & store to `arxiv_vector_store03`.

✅ Output:
```
Vector store for confidential data created and persisted successfully.
```
...
Use the helper in `retreive.py`:

```python
from retreive import query_api

result = query_api("What is Temporal-Spatial Mapping in action recognition?", k=2)
...
- `index.py` → Index CSV into Qdrant.  
- `retreive.py` → Search & answer queries using RAG.  
- Returns: **Answer + Context + Sources**.  
- Perfect for **research Q&A on academic abstracts**.
